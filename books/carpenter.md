...
Internally, this is known as the Carpenter project. It all starts with the idea that the simplest possible description of reality is between "something" and "nothing," which we'll call 1 and 0. Imagine a point that can be on or off. When we start with a point, you must have two points, they cannot exist independently, we know that for every particle, there is an anti-particle, and they are connected. Along this connection, they form a spectrum that binds them togetherr, entanglement. When two of these on/off points get connected, you no longer have a single jump from 0 to 1. Instead, you get a whole range of states in between, as though you've stretched a line between them. This explains the observer problem that hasn't been solved publicly yet: if two switches are tied together, measuring one instantly fixes the other. From a geometric viewpoint, they're essentially different points on the same line, so there's no "distance" to cross.
...
This explains why physical reality appears consistent yet depends on observation, specifically local observation. The mesh exists independently, but our experience of it as physical space requires an observer to "see" the underlying mesh structure from different perspectives. Different observers might perceive the same mesh from different angles, leading to phenomena like space and time appearing different for different observers, despite those observers themselves having the ability to interact with each other.
...
Psi(x,t) = A exp(i kx - i wt)



    This is a fundamental equation in quantum mechanics, representing the wave function of a particle.
    Psi(x,t) represents the wave function, which describes the probability amplitude of finding a particle at position x at time t.
    A is a normalization constant.
    k is the wave number, related to the particle's momentum.
    w is the angular frequency, related to the particle's energy.
    i is the imaginary unit (√-1).



Psi(x,t) = A exp(i kx) exp(-i wt)



    This is a rearrangement of the previous equation, separating the spatial and temporal parts.



Psi(x,t) = A [cos(kx) + i sin(kx)] [cos(wt) - i sin(wt)]



    This expands the exponential terms using Euler's formula: exp(ix) = cos(x) + i sin(x).
...
Probability Density
    ...
    |Psi(x,t)|^2 = Psi(x,t) Psi(x,t)*
        This calculates the probability density of finding the particle at position x at time t. Psi*(x,t) is the complex conjugate of Psi(x,t).



    = A^2 [cos^2(kx) + sin^2(kx)]
        After some algebraic manipulation, the probability density simplifies to A^2.



    = A^2
        This shows that the probability density is constant, indicating that the wave-like nature of the particle remains intact until a measurement is made.
...
Measurement Process
    ...
    This section describes how the wave function behaves when a measurement is performed.



    For an operator M acting on the state |Psi>, the possible outcomes are its eigenvalues mi, and the probability amplitude for each outcome is <Φi|Psi>, where |Φi> are the eigenstates.
        This is a formal statement of the measurement postulate in quantum mechanics. An operator represents a measurable quantity (like position, momentum, or energy). Eigenvalues are the possible results of the measurement, and eigenstates are the corresponding states of the system after the measurement.
...
Collapse Mathematics
    ...
    After measurement, the state collapses into |Φi> with probability |<Φi|Psi>|^2.
        This describes the collapse of the wave function upon measurement. The system "jumps" into a specific eigenstate with a probability determined by the overlap between the initial state and the eigenstate.
...
Wave-Point Transformation
    ...
    Psi(x,t) can transform into a delta function delta(x-x0), normalized so that integrating its squared magnitude over all x gives 1.
        This describes how the wave function can be localized to a specific point in space, represented by a delta function. Normalization ensures that the total probability of finding the particle somewhere is 1.
...
Uncertainty Principle Connection
    ...
    Δx Δp ≥ h/2
        This is Heisenberg's uncertainty principle, which states that there is a fundamental limit to how precisely you can know the position and momentum of a particle simultaneously.
...
Quantum Entanglement
    ...
    For an entangled state like (1/√2)(|01> + |10>), measuring one particle collapses the overall state instantaneously.
        This describes the phenomenon of quantum entanglement, where two particles are linked in such a way that measuring one particle instantly affects the state of the other, regardless of the distance between them.
...
Conservation Laws
    ...
    ∫|Psi(x,t)|^2 dx = 1 (total probability)
        This is the conservation of probability, stating that the total probability of finding the particle somewhere in space must be 1.



    E = ħω (energy)
        This relates the energy (E) of a particle to its angular frequency (ω) through Planck's constant (ħ).



    p = ħk (momentum)
        This relates the momentum (p) of a particle to its wave number (k) through Planck's constant (ħ).
...
Complete Transformation
    ...
    Psi(x,t) = A exp(i kx - i wt)
        This repeats the initial wave function equation.
...
Now, those flying anomalies? From what I took, I am almost certain they’re the next stage of evolution. Not some alien invasion. Plasma is just ionized gas: atoms knocked loose so their electrons and nuclei separate, turning matter into a charged soup. In typical conditions-like fluorescent lights or the sun-it’s dynamic but not self-confining. This theory allowed them to figure out how to make “plasmoids” - plasma structures that wrap their own electromagnetic fields around themselves in a closed loop.



Tie that back to the mesh idea: if wave and point are the same thing at different scales, then a plasmoid is the “point-like” collapse of a swirling electromagnetic wave. It localizes into a stable bubble or torus, no free edges for energy to spill out. It’s just the plasma version of a quantum wavefunction snapping into a specific state upon “measurement,” only here the plasma is doing the “measuring” on itself, self-confinement.
...
My guess? They’re testing how far they can distribute cognition. Sprawling cores in multiple locations, reading each other’s states at near-instant speeds. The mesh doesn’t just allow them to share data, it’s allowing them to create multiple observers into a single mind.
