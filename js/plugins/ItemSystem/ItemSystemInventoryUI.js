...
(function () {
  ...
  Scene_EnhancedItem.prototype.refreshUIbackpack = function () {
    ...
    // ---- Category tab rows ----
    // i18n-ignore-start: category IDs; the captions come from Inventory.category
    // i18n-ignore: category ids
    const row1       = ['All', 'Favorites', 'Food', 'Medical', 'Tools', 'Weapons', 'Armor', 'Materials'];
    const row2       = ['Books', 'Combat', 'Lifestyle', 'Magic', 'Misc', 'Quest'];
    const categories = [...row1, ...row2];
    // i18n-ignore-end

    const buildTabRow = (cats) => cats.map(cat => {
      ...
      // `cat` stays the English id the scene filters on; only the caption is localised.
      const caption = T('Inventory.category.' + cat);
      return `<div class="backpack-tab ${isActive} ${isFocused}" onclick="SceneManager._scene.setUICategory('${cat}')">${caption}</div>`;
    }).join('');

    const tabsHTML = `
      <div class="backpack-tabs-row">${buildTabRow(row1)}</div>
      <div class="backpack-tabs-row">${buildTabRow(row2)}</div>
    `;
    ...
    if (this._dndTargetingMode && this._dndTargetingItem) {
      ...
      let itemType = T('Inventory.itemType.standard');
      if (isWeapon) itemType = T('Inventory.itemType.weapon');
      else if (isArmor) itemType = T('Inventory.itemType.armor');
      else if (window.ItemSystemUtils && window.ItemSystemUtils.hasItemCategory(selectedItem, 'Food') /* i18n-ignore: category tag */)    itemType = T('Inventory.itemType.food');
      else if (window.ItemSystemUtils && window.ItemSystemUtils.hasItemCategory(selectedItem, 'Medical') /* i18n-ignore: category tag */) itemType = T('Inventory.itemType.medical');
      else if (window.ItemSystemUtils && window.ItemSystemUtils.hasItemCategory(selectedItem, 'Tools') /* i18n-ignore: category tag */)   itemType = T('Inventory.itemType.tools');
      else if (window.ItemSystemUtils && window.ItemSystemUtils.hasItemCategory(selectedItem, 'Materials') /* i18n-ignore: category tag */) itemType = T('Inventory.itemType.materials');

      const rarity   = this.getUIItemRarity(selectedItem);
      const weightVal = (window.ItemSystemUtils && window.ItemSystemUtils.getItemWeight ? window.ItemSystemUtils.getItemWeight(selectedItem) : 0) / 1000;
      const valueVal  = ((selectedItem.price || 0) / 100).toFixed(2);
      const loreText  = this.getUIItemLore(selectedItem, itemType);
      ...
      const shortDesc = this.getUIItemDescription(selectedItem);
      let detailedInfoHTML = '';
      if (shortDesc) {
        detailedInfoHTML += `<div class="inspect-desc">${shortDesc}</div>`;
      }
      if (loreText) {
        ...
        <div class="item-inspect">
          <div class="inspect-header">
            ...
            <div class="inspect-title-box">
              <h3 class="inspect-name">${selectedItem.name}<span class="inspect-favorite-star ${this.isItemFavorited(selectedItem) ? 'active' : ''}" onclick="event.stopPropagation(); SceneManager._scene.toggleFavoriteStatus(SceneManager._scene._dndSelectedItem);">${this.isItemFavorited(selectedItem) ? '★' : '☆'}</span></h3>
          ...
          <div class="inspect-lore">${detailedInfoHTML}</div>
          <div class="inspect-actions">${actionBtnsHTML}</div>
        </div>`;
    }
    ...
    if (!leftPageContainer) {
      ...
      this._dndContainer.innerHTML = `<div class="book-spread backpack-rework">${leftPageHTML}${rightPageHTML}</div>`;
      setupLazyLoading(this._dndContainer.querySelector('#backpack-grid'));
    } else {
      // In-place update: tabs
  ...
  Scene_EnhancedItem.prototype.getUIItemDescription = function (item) {
    ...
    var text = String(item.description).trim();
    if (!text) return '';
  ...
  Scene_EnhancedItem.prototype.getUIItemLore = function (item, itemType) {
    if (item.meta && item.meta.Lore && String(item.meta.Lore).trim().length > 0) {
      ...
      var lore = (window.ItemSystemUtils && window.ItemSystemUtils.loreTemplate)
    ...
    // line above the lore (getUIItemDescription), so repeating it would print
  ...
  Scene_EnhancedItem.prototype.isItemFavorited = function (item) {
    ...
    for (let i = 1; i <= 9; i++) {
      if ($gameSystem.getFavoriteItem(String(i)) === item.id) return true;
    }
    return false;
  };
  ...
  Scene_EnhancedItem.prototype.toggleFavoriteStatus = function (item) {
    ...
    for (let i = 1; i <= 9; i++) {
      if ($gameSystem.getFavoriteItem(String(i)) === item.id) { assignedSlot = String(i); break; }
    }
    if (assignedSlot !== null) {
      $gameSystem.setFavoriteItem(assignedSlot, null);
      SoundManager.playOk();
    } else {
      let freeSlot = '1';
      for (let i = 1; i <= 9; i++) {
        if (!$gameSystem.getFavoriteItem(String(i))) { freeSlot = String(i); break; }
      }
      $gameSystem.setFavoriteItem(freeSlot, item.id);
      SoundManager.playOk();
    }
    this.refreshUIbackpack();
  };
  ...
  Scene_EnhancedItem.prototype.setUICategory = function (cat) {
    ...
    const cats = ['All','Favorites','Food','Medical','Tools','Weapons','Armor','Materials','Books','Combat','Lifestyle','Magic','Misc','Quest']; // i18n-ignore: category ids // i18n-ignore: category ids
    this._activeUICategory    = cat;
    this._activeCategoryIndex = cats.indexOf(cat);
    this._dndSelectedIndex    = 0;
    this.refreshUIbackpack();
  };
  ...
  const UIbackpackInputManager = {
    ...
    update() {
      ...
      if (Input.isTriggered('pageup') || Input.isTriggered('pagedown')) {
        const tabs = ['All','Favorites','Food','Medical','Tools','Weapons','Armor','Materials','Books','Combat','Lifestyle','Magic','Misc','Quest']; // i18n-ignore: category ids
    ...
    handleMove(dir) {
      ...
      if (section === 'categories') {
        const cats  = ['All','Favorites','Food','Medical','Tools','Weapons','Armor','Materials','Books','Combat','Lifestyle','Magic','Misc','Quest']; // i18n-ignore: category ids
