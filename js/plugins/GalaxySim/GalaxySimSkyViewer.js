...
(() => {
    ...
    class Sprite_StarMap extends PIXI.Container {
        constructor(isTelescope = false) {
            ...
            // Scratch point + last-pointer cache reused by _updateHover so it
            // neither allocates nor rescans the constellations when idle.
            this._hoverScratch = new PIXI.Point();
            this._hoverLocal = new PIXI.Point();
            this._lastHoverX = -1;
            this._lastHoverY = -1;
        ...
        _createButtons() {
            this._westernButton = this._createButton(T('Galaxy.sky.european'), 10, 10, () => this._onWesternClick());
            this._chineseButton = this._createButton(T('Galaxy.sky.chinese'), 130, 10, () => this._onChineseClick());
        ...
        _createHoverText() {
            ...
            this._createControlHint();
        }

        // Everything here is already reachable with a pad (the stick and D-pad
        // pan, LB/RB zoom, X switches the sky, B closes); the line just says so,
        // naming the buttons when a controller is plugged in and the keys when
        // it is not.
        _createControlHint() {
            this._controlHint = new PIXI.Text("", {
                fontFamily: "Arial",
                fontSize: 15,
                fill: 0xAAAAAA,
                stroke: 0x000000,
            ...
            this._controlHint.anchor.set(0.5, 1);
            this._controlHint.x = this.width / 2;
            this._controlHint.y = this.height - 12;
            this._controlHint.zIndex = 10;
            this.addChild(this._controlHint);
            this._refreshControlHint();
        }

        _refreshControlHint() {
            if (!this._controlHint) return;
            const pad = !!(window.AnalogStickInput && AnalogStickInput.hasPad &&
                AnalogStickInput.hasPad());
            if (pad === this._hintPad && this._controlHint.text) return;
            this._hintPad = pad;
            this._controlHint.text = T(pad ? 'Galaxy.sky.hintPad' : 'Galaxy.sky.hint');
        }
        ...
        _updateZoom() {
            ...
            // LB/RB on a pad. On a keyboard pagedown sits on W, which also pans
            // up here, so the rebindable zoomOut (E) is accepted alongside it.
            if (Input.isPressed('pageup')) {
                // Zoom towards center
                this._applyZoomMultiplicative(1.02);
            }

            if (Input.isPressed('pagedown') || Input.isPressed('zoomOut')) {
                // Zoom towards center
        ...
        _updateHover() {
            ...
            // Early-out when nothing that affects the hover result has changed:
            // the pointer, the sky pan, and the zoom all being identical means
            // the previously-resolved name still holds.
            const tx = TouchInput.x, ty = TouchInput.y;
            const cx = this._skyContainer.x, cy = this._skyContainer.y;
            const zoom = this._zoomLevel;
            if (tx === this._lastHoverX && ty === this._lastHoverY &&
                cx === this._lastHoverCx && cy === this._lastHoverCy &&
                zoom === this._lastHoverZoom) {
                return;
            }
            this._lastHoverX = tx; this._lastHoverY = ty;
            this._lastHoverCx = cx; this._lastHoverCy = cy;
            this._lastHoverZoom = zoom;

            const touchPos = this._hoverScratch;
            touchPos.set(tx, ty);
            const localPos = this._skyContainer.worldTransform.applyInverse(touchPos, this._hoverLocal);
