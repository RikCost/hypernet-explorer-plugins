...
(() => {
  ...
  class PlanetRenderer {
    ...
    drawPlanet(ctx, x, y, radius, planet, seed, time, sunScreen) {
      const clampedRadius = Math.min(radius, PLANET_MAX_SIZE);

      // Try true 3D rendering first (lit sphere, clouds, rings, terminator).
      // Falls through to the 2D path below if WebGL/THREE is unavailable.
      if (
        clampedRadius >= PLANET_MIN_SIZE &&
        window.GalaxySim &&
        window.GalaxySim.Renderer3D &&
        window.GalaxySim.Renderer3D.available()
      ) {
        const ok = window.GalaxySim.Renderer3D.renderPlanet(
          ctx, x, y, clampedRadius, planet, seed, time, sunScreen
        );
        if (ok) {
          if (this.hasAtmosphere(planet.type)) {
            this.drawAtmosphereGlow(ctx, x, y, clampedRadius, planet);
          }
          return;
        }
      }
    ...
    // Screen-space atmospheric rim glow drawn on top of the 3D sphere.
    drawAtmosphereGlow(ctx, x, y, radius, planet) {
      const atmosColor = this.getAtmosphereColor(planet.type);
      const rgbStr = atmosColor.replace("rgb(", "").replace(")", "");
      ctx.save();
      ctx.globalCompositeOperation = "lighter";
      const glow = ctx.createRadialGradient(x, y, radius * 0.85, x, y, radius * 1.25);
      glow.addColorStop(0, `rgba(${rgbStr}, 0)`);
      glow.addColorStop(0.7, `rgba(${rgbStr}, 0.18)`);
      glow.addColorStop(0.92, `rgba(${rgbStr}, 0.30)`);
    ...
    lightenColor(color, factor) {
      ...
      return `rgb(${Math.round(Math.min(255, rgb.r * factor))}, ${Math.round(Math.min(255, rgb.g * factor))}, ${Math.round(Math.min(255, rgb.b * factor))})`;
    }
