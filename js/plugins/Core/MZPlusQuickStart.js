...
($=> {
    const filter = $plugins.filter(function (p) {
        return p.description.includes('<MZPlusQuickStart>') && p.status;
    })[0];
    // Guard filter too: if the description tag desyncs, filter(...)[0] is
    // undefined and dereferencing .parameters throws at load.
    const params = filter && filter.parameters;
    if (!params) {
