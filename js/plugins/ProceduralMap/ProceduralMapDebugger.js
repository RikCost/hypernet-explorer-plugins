...
(() => {
  ...
  function isActive() {
    return DEBUG_MODE && $gameSystem && $gameSystem._isProceduralMapDebuggerActive;
  }
  ...
  // Local getBiomeByName/createSeededRandom copies used to back this plugin's
  // own specialBiomes roll; that roll now delegates to ProcGenUtils, so the
  // duplicates are gone.
  ...
  function getActualBiomeForWorldCoordinates(worldTileId, worldX, worldY) {
    ...
    // Delegate to the shared roll so the overlay label matches the map the
    // generator actually builds. This used to re-implement the roll with a
    // different seed (procGenData.seed + additive coord mixing), which made the
    // tile label disagree with both the generated map and the travel HUD.
    const resolve = window.ProcGenUtils?.resolveSpecialBiome;
    if (resolve) return resolve(baseBiomeName, worldX, worldY);
  ...
  Scene_Map.prototype.createSpriteset = function () {
    ...
    // Always create the overlay sprite while the plugin is enabled; the sprite's
    // own update() gates on isActive() for visibility. This way toggling the
    // debugger from the Sandbox menu takes effect immediately on return to the
    // map, instead of only when the debugger was already ON before transferring
    // onto the map (createSpriteset does not re-run when popping back a scene).
    if (DEBUG_MODE && DISPLAY_ON_SCREEN) {
