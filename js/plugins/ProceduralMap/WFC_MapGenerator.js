...
(() => {
    ...
    class WaveFunctionCollapse {
        constructor(patternSize, wrapPatterns, fallbackTileId, seed) {
            ...
            // Seeded RNG so cell collapse and tie-breaking are deterministic and
            // derived from the world seed, matching the overland pipeline. Falls
            // back to Math.random when no seed or ProcGenUtils is available. The
            // stream is stateful so contradiction restarts still diverge.
            this.rng = (typeof seed === 'number' && window.ProcGenUtils && window.ProcGenUtils.createSeededRandom)
                ? window.ProcGenUtils.createSeededRandom(seed >>> 0)
                : Math.random;
        }
        ...
        generate(width, height) {
            ...
            while (remainingCells > 0 && iterations < maxIterations) {
                ...
                const chosenIndex = minEntropyIndices[Math.floor(this.rng() * minEntropyIndices.length)];
        ...
        generateFallback(width, height) {
            ...
            const grid = Array(width * height).fill().map(() => {
                return {
                    collapsed: true,
                    options: [Math.floor(this.rng() * this.patterns.length)]
                };
            });
        ...
        updateOptions(cell, neighborCell, direction) {
            ...
            if (newOptions.length === 0 && neighborCell.options.length > 0) {
                ...
                newOptions.push(neighborCell.options[Math.floor(this.rng() * neighborCell.options.length)]);
            }
    ...
    function generateMap(width, height, targetMapId, specificPatternMapIds, customFallbackTileId) {
        ...
        // Derive a deterministic RNG seed from the world seed (distinct stream per
        // target map) so WFC output is reproducible and consistent with the
        // overland pipeline. Left undefined (Math.random) if ProcGenUtils is absent.
        let wfcSeed;
        if (window.ProcGenUtils && window.ProcGenUtils.getWorldSeed && window.ProcGenUtils.hashCoords) {
            wfcSeed = window.ProcGenUtils.hashCoords(window.ProcGenUtils.getWorldSeed(), (targetMapId | 0), 0x7fc0);
        }

        // Create WFC algorithm instance
        const wfc = new WaveFunctionCollapse(patternSize, wrapPatterns, useFallbackTileId, wfcSeed);
    ...
    if (Utils.isOptionValid('test')) {
        ...
        Scene_Menu.prototype.commandWfcGenerate = function() {
            ...
            optionsWindow.setHandler('ok', function() {
                ...
                $gameMessage.add(T('WFC.complete'));
        ...
        Window_WfcOptions.prototype.makeCommandList = function() {
            this.addCommand(T('WFC.width', { value: this._width }), 'width');
            this.addCommand(T('WFC.height', { value: this._height }), 'height');
            this.addCommand(T('WFC.targetMapId', { value: this._targetMapId }), 'target');
            this.addCommand(T('WFC.useSpecificPattern', { value: this._useSpecificPattern ? T('WFC.yes') : T('WFC.no') }), 'pattern_toggle');
            
            if (this._useSpecificPattern) {
                this.addCommand(T('WFC.patternMapId', { value: this._patternMapId }), 'pattern_id');
            }
            
            this.addCommand(T('WFC.fallbackTileId', { value: this._fallbackTileId }), 'fallback');
            this.addCommand(T('WFC.generate'), 'ok');
        };
