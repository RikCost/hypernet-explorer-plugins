...
(() => {
    ...
    Game_System.prototype.generateRandomRoomsDungeon = function() {
        ...
        while (this._dungeonRooms.length < maxRooms && attempts < maxAttempts) {
            ...
            // Random room size, clamped to fit inside the dungeon bounds (with a
            // 1 tile border) like the BSP path, so a large roomMaxWidth in a small
            // dungeon cannot produce a negative placement range / out-of-bounds writes.
            const maxRoomWidth = Math.max(roomMinWidth, Math.min(roomMaxWidth, dungeonWidth - 2));
            const maxRoomHeight = Math.max(roomMinHeight, Math.min(roomMaxHeight, dungeonHeight - 2));
            const roomWidth = Math.floor(Math.random() * (maxRoomWidth - roomMinWidth + 1)) + roomMinWidth;
            const roomHeight = Math.floor(Math.random() * (maxRoomHeight - roomMinHeight + 1)) + roomMinHeight;

            // Random position (with 1 tile border). Guard the range so a room as
            // wide/tall as the dungeon still yields a valid, in-bounds origin.
            const roomX = Math.floor(Math.random() * Math.max(1, dungeonWidth - roomWidth - 2)) + 1;
            const roomY = Math.floor(Math.random() * Math.max(1, dungeonHeight - roomHeight - 2)) + 1;
