...
Sprite_Compass.prototype.findTargetEvents = function() {
    $gameMap.events().forEach(event => {
        ...
        if (data.name === "NextFloor") {
            this._nextFloorEvent = event;
        }
        if (data.name === "PrevFloor") {
...
// Redraw the entire compass, but only when the drawn state actually changed.
// The hands depend solely on the player's tile position and the target events'
// tile positions (plus visibility), so skip the full clear+redraw when none of
// those changed since the last frame.
Sprite_Compass.prototype.redrawCompass = function() {
    const nx = this._nextFloorEvent ? this._nextFloorEvent.x : null;
    const ny = this._nextFloorEvent ? this._nextFloorEvent.y : null;
    const px = this._prevFloorEvent ? this._prevFloorEvent.x : null;
    const py = this._prevFloorEvent ? this._prevFloorEvent.y : null;
    const vis = this.visible ? 1 : 0;
    // Numeric field compares avoid building/parsing a joined string every frame.
    if (this._lastVis === vis &&
        this._lastPlayerX === $gamePlayer.x && this._lastPlayerY === $gamePlayer.y &&
        this._lastNX === nx && this._lastNY === ny &&
...
// Re-scan for target events whenever a new map is loaded, since the Scene_Map
// (and its compass sprite) persists across player transfers.
var _Scene_Map_onMapLoaded = Scene_Map.prototype.onMapLoaded;
Scene_Map.prototype.onMapLoaded = function() {
    _Scene_Map_onMapLoaded.call(this);
    if (this._compassSprite) {
        this._compassSprite._eventsFound = false;
        this._compassSprite._framesWaited = 0;
        this._compassSprite._nextFloorEvent = null;
        this._compassSprite._prevFloorEvent = null;
    }
};

// The compass sprite is a child of the scene (addChild in createCompassSprite),
// so Scene_Base.updateChildren already calls its update() once per frame. A
// manual second update() call here would draw/scan the compass twice per frame.
