...
(() => {
    ...
    Sprite_Passability.prototype.initialize = function() {
        ...
        // Bitmap is created lazily on first toggle-on (see update).
        if (this.visible) {
            this.createBitmap();
            this.refresh();
        }
    ...
    Sprite_Passability.prototype.update = function() {
        ...
        this.visible = Input.IsDebugVisible();
        // Skip all scanning/redraw work while the overlay is hidden.
        if (!this.visible) return;
        if (!this.bitmap) this.createBitmap();
        //
        ...
        if (!wasVisible || floorDX != this._currOX || floorDY != this._currOY) {
    ...
    Sprite_Passability.prototype.drawTile = function(x,y) {
        ...
        if (!d || !l || !r || !u) {
            ...
            //
            // checkPassageLayer builds passTile from allTiles() = [...tileEvents, L3, L2, L1, L0]
            // (events first, variable count; the 4 layered tiles are always the last 4, reversed).
            const n = passTile.length;
            const L0 = passTile[n - 1];
            const L1 = passTile[n - 2];
            const L2 = passTile[n - 3];
            const L3 = passTile[n - 4];
            let eventImpassable = false;
            for (let ti = 0; ti < n - 4; ti++) {
                if (passTile[ti] === false) { eventImpassable = true; break; }
            }
