...
`;
    ...
    class Window_KanbanBoard extends Window_Selectable {
        constructor(rect) {
            super(rect);
            this._columns = ['todo', 'inProgress', 'done', 'failed'];
            this._columnTitles = T.list('Kanban.columnTitles');
        ...
        updateKeyboardInput() {
            ...
            // Constrain selection
            this._keyboardSelection.column = Math.max(0, Math.min(this._columns.length - 1, this._keyboardSelection.column));
        ...
        handleMouseDown(pos, currentTime) {
            ...
            if (note) {
                // Prevent any interaction with done/failed quests
                if (note.quest.column === 'done' || note.quest.column === 'failed') {
                    return;
                }
        ...
        handleDrop() {
            ...
            // Prevent moving to done/failed columns (silently)
        ...
        getColumnAt(x) {
            const columnWidth = this.contents.width / this._columns.length;
            const idx = Math.floor(x / columnWidth);
            return this._columns[idx] || null;
        }
        ...
        drawColumns() {
            ...
            // Draw book cover deep mahogany leather frame
            this.contents.fillRect(0, 0, this.contents.width, this.contents.height, '#2b1008');
            this.contents.strokeRect(2, 2, this.contents.width - 4, this.contents.height - 4, '#58180d', 4);

            for (let i = 0; i < this._columns.length; i++) {
                const x = i * columnWidth;

                // Draw warm parchment page for the column
                this.contents.fillRect(x + 8, 8, columnWidth - 16, this.contents.height - 16, '#f5ebd0');

                // Draw elegant double border line in dark ink
                this.contents.strokeRect(x + 12, 12, columnWidth - 24, this.contents.height - 24, '#58180d', 2);
                this.contents.strokeRect(x + 15, 15, columnWidth - 30, this.contents.height - 30, '#805d3f', 1);

                // Draw accent title banner
                this.contents.fillRect(x + 16, 16, columnWidth - 32, headerHeight - 12, '#e6d4aa');
                this.contents.strokeRect(x + 16, 16, columnWidth - 32, headerHeight - 12, '#58180d', 1);

                // Column title in Lora serif
                this.contents.fontFace = '"Lora", "Lora", serif';
                this.contents.fontSize = 28;
                this.contents.fontBold = true;
                this.contents.textColor = '#58180d';
                this.contents.drawText(this._columnTitles[i], x, 14, columnWidth, headerHeight - 12, 'center');
        ...
        drawStickyNote(note, isDragging = false) {
            ...
            const alpha = isDragging ? 0.75 : 1.0;

            // Draw card drop shadow
            if (!isDragging) {
                this.contents.fillRect(note.x + 3, note.y + 3, note.width, note.height, 'rgba(0,0,0,0.15)');
            }

            this.contents.paintOpacity = alpha * 255;

            // Background color
            let cardBg = quest.color;
            if (this._hoveredNote === note && this._keyboardMode && !isDragging) {
                cardBg = '#ebd596'; // Gold highlight
            }

            this.contents.fillRect(note.x, note.y, note.width, note.height, cardBg);

            // Border ink
            let borderColor = '#58180d';
            let borderWidth = 2;

            if (this._hoveredNote === note && !isDragging) {
                borderColor = '#b8860b'; // Gold highlight
                borderWidth = 3;
            }

            this.contents.strokeRect(note.x, note.y, note.width, note.height, borderColor, borderWidth);

            // Done/Failed watermark stamp
            if (quest.column === 'done' || quest.column === 'failed') {
                this.contents.paintOpacity = 130;
                this.contents.fontFace = '"Lora", serif';
                this.contents.fontSize = 14;
                this.contents.textColor = '#8b263e';
                const stamp = quest.column === 'done' ? 'RESOLVED' : 'FAILED';
                this.contents.drawText(stamp, note.x + note.width - 95, note.y + note.height - 24, 85, 20, 'right');
            }

            // Text rendering in Lora serif font
            this.contents.paintOpacity = 255;
            this.contents.fontFace = '"Lora", "Lora", serif';
            this.contents.fontSize = 16;
            this.contents.fontBold = true;
            this.contents.textColor = quest.column === 'done' ? '#5a554a' : '#2b251d';

            this.drawTextEx(quest.title, note.x + 12, note.y + 12, note.width - 24);
            this.contents.fontBold = false;
        }
    }

    // Window for Quest Details (D&D Parchment Scroll style)
    class Window_QuestDetails extends Window_Selectable {
        constructor(rect) {
            super(rect);
            this._quest = null;
            this.setBackgroundType(2); // transparent/clean
        }
        ...
        drawItem(index) {
            ...
            this.contents.textColor = '#805d3f';
            this.contents.drawText(update.date, rect.x + 10, rect.y, rect.width - 20, 'left');

            // Draw update text in dark ink
            this.contents.fontSize = 16;
            this.contents.textColor = '#2b251d';
            const textY = rect.y + 18;
            this.drawTextEx(update.text, rect.x + 10, textY, rect.width - 20);
        }
        ...
        drawAllItems() {
            ...
            // Draw wooden-leather frame
            this.contents.fillRect(0, 0, this.contents.width, this.contents.height, '#2b1008');
            this.contents.strokeRect(3, 3, this.contents.width - 6, this.contents.height - 6, '#58180d', 3);

            // Draw parchment sheet
            this.contents.fillRect(10, 10, this.contents.width - 20, this.contents.height - 20, '#faf2d3');
            this.contents.strokeRect(14, 14, this.contents.width - 28, this.contents.height - 28, '#805d3f', 1);

            // Draw quest name title banner
            this.contents.fillRect(15, 15, this.contents.width - 30, 45, this._quest.color);
            this.contents.strokeRect(15, 15, this.contents.width - 30, 45, '#58180d', 1);
            ...
            this.contents.fontBold = true;
            this.contents.textColor = '#58180d';
            this.contents.drawText(this._quest.title, 25, 20, this.contents.width - 50, 36, 'left');
            this.contents.fontBold = false;
        ...
        itemRect(index) {
            ...
            rect.y += 65; // Offset for header title banner
            rect.x += 10;
            rect.width -= 20;
            return rect;
        }
