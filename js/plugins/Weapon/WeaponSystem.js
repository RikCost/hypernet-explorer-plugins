...
(() => {
  ...
  DataManager.extractWeaponSystemData = function (weapon) {
    ...
    if (animMatch) {
      ...
      weapon.weaponAnimations = ["Swing"];  // i18n-ignore  animation clip name
  ...
  Game_Actor.prototype.getShieldSpriteData = function () {
    ...
    return {
      ...
      animations: randomArmor.weaponAnimations || ["Swing"],  // i18n-ignore  animation clip name
    };
  };
  ...
  Game_Actor.prototype.getWeaponSpriteData = function () {
    ...
    if (weapons.length === 0) {
      return {
        sprite: "Unarmed1",
        frames: ["Unarmed1"],
        animations: ["Thrust"], // Thrust animation for punch  // i18n-ignore  animation clip name
      };
    }
    ...
    return {
      ...
      animations: weapon.weaponAnimations || ["Swing"],  // i18n-ignore  animation clip name
    };
  };
  ...
  Game_Actor.prototype.reloadBullets = function () {
    ...
    if (weapons.length > 0) {
      ...
      if (ConfigManager.weaponSprites3D && window.Sprite_3DWeapon) {
  ...
  Sprite_WeaponFPS.prototype.initialize = function (mirrored = false) {
    ...
    // Slide-in transition state (weapon enters from the right when switching actor)
    this._sliding = false;
    this._slideOffsetX = 0;
    this._slideStartTime = 0;
    this._slideDuration = 200;
    this._slideStartOffset = 0;
    this.resetPosition();
  };
  // Start a smooth slide-in from the right edge of the screen.
  // Used when the active actor changes so the new weapon glides into view.
  Sprite_WeaponFPS.prototype.startSlideIn = function () {
    if (!this.visible) return;
    this._sliding = true;
    this._slideStartTime = Date.now();
    const spriteWidth =
      (this.bitmap && this.bitmap.width ? this.bitmap.width : 200) *
      Math.abs(this.scale.x || 1);
    this._slideStartOffset = Math.max(0, Graphics.width - this._baseX) + spriteWidth;
    this._slideOffsetX = this._slideStartOffset;
  };
  ...
  Sprite_WeaponFPS.prototype.setWeapon = function (actor) {
    ...
    this._currentSpriteFrame = 0;
    this._availableAnimations = weaponData.animations || ["Swing"];  // i18n-ignore  animation clip name
  ...
  // Tint the weapon with the acting actor's most recently applied shimmer
  // state (<weaponShimmer>); the color comes from that state's <Hex> tag.
  Sprite_WeaponFPS.prototype._updateWeaponShimmer = function () {
    let color = null;
    if (this.visible && this.bitmap && this.parent &&
        this.parent.getCurrentBattleActor) {
      const actor = this.parent.getCurrentBattleActor(false);
      if (actor) color = actor.getLatestWeaponShimmerColor();
    }
    if (color == null) {
    ...
    const b = color & 0xff;
    // Gentle pulse so the tint reads as a shimmer rather than a flat overlay.
    const pulse = (Math.sin(Date.now() / 180) + 1) / 2; // 0..1
    const alpha = Math.round(100 + pulse * 80); // 100..180
    this.setBlendColor([r, g, b, alpha]);
  };
    ...
    if (this._mirrored) {
      ...
      if (isDualWielding && weapons[1]) {
        ...
        if (leftWeapon.wtypeId === 11) {
          this._animationFrames = MovementKeyFrame["Thrust"];  // i18n-ignore  MovementKeyFrame clip name
      ...
      if (animName.toLowerCase() === "static") {
        ...
        this._animationFrames = MovementKeyFrame["Swing"];  // i18n-ignore  MovementKeyFrame clip name
      } else {
  ...
  Sprite_WeaponFPS.prototype.update = function () {
    ...
    this._updateWeaponShimmer();

    // Compute slide-in offset (decays from offscreen-right to 0)
    if (this._sliding) {
      const t = (Date.now() - this._slideStartTime) / this._slideDuration;
      if (t >= 1) {
        this._sliding = false;
        this._slideOffsetX = 0;
      } else {
        this._slideOffsetX =
          this._slideStartOffset * (1 - this._easeOutCubic(t));
      }
    }
    ...
    // Apply slide-in offset on top of whatever positioning ran above
    if (this._slideOffsetX) {
      this.x += this._slideOffsetX;
    }
  ...
  Sprite_ShieldFPS.prototype.setShield = function (actor) {
    ...
    this._currentSpriteFrame = 0;
    this._availableAnimations = shieldData.animations || ["Swing"];  // i18n-ignore  animation clip name
  ...
  Spriteset_Battle.prototype.createWeaponSprite = function () {
    ...
    // constructs a Spriteset_Battle, but skip the FPS weapon overlay here too
    // in case any other code path ever instantiates one while it's active.
  ...
  Spriteset_Battle.prototype.getSpecialWeaponSprite = function(type, isLeft = false) {
    const key = isLeft ? `_${type}SpriteLeft` : `_${type}Sprite`;  // i18n-ignore  sprite cache key
    if (!this[key]) {
      switch(type) {
  ...
  Spriteset_Battle.prototype.updateWeaponSprite = function () {
    ...
    // Hide the held weapon/shield visuals ONLY in card combat mode
    // (RoguelikeCardSystem, Switch 45, locked at character creation): there
    // attacks are cards, not weapon swings. Outside card mode the held 2D FPS
    // weapon/shield and 3D weapon models render normally.
    const cardMode = window.isCardCombatMode ? window.isCardCombatMode() : $gameSwitches.value(45);
    if (cardMode) {
      this._weaponSprite.visible = false;
      if (this._weaponSpriteLeft) this._weaponSpriteLeft.visible = false;
      if (this._shieldSprite) this._shieldSprite.visible = false;
    ...
    if (ConfigManager.weaponSprites3D) {
      this._weaponSprite.visible = false;
      if (this._weaponSpriteLeft) this._weaponSpriteLeft.visible = false;
      if (this._shieldSprite) this._shieldSprite.visible = false;
      ...
      if (window.Sprite_3DWeapon) {
        ...
        if (actor) {
          ...
          // Right Hand Weapon
          const rightWeapon = weapons[0];
          if (rightWeapon) {
            if (!this._3dWeaponSprites['right'] || this._3dWeaponSprites['right']._weapon !== rightWeapon) {
              if (this._3dWeaponSprites['right']) {
                this._3dWeaponSprites['right'].terminate();
              }
              const spriteX = getScaledWeaponX(false);
              const spriteY = getScaledWeaponY();
              this._3dWeaponSprites['right'] = new Sprite_3DWeapon(rightWeapon, spriteX, spriteY);
            }
            if (this._3dWeaponSprites['right']._model) {
              this._3dWeaponSprites['right']._model.visible = true;
              this._3dWeaponSprites['right']._visible = true;
            }
          } else {
            if (this._3dWeaponSprites['right']) {
              this._3dWeaponSprites['right'].terminate();
              delete this._3dWeaponSprites['right'];
            }
          }
          ...
          if (leftWeapon) {
            if (!this._3dWeaponSprites['left'] || this._3dWeaponSprites['left']._weapon !== leftWeapon) {
              if (this._3dWeaponSprites['left']) {
                this._3dWeaponSprites['left'].terminate();
              }
              const spriteX = getScaledWeaponX(true);
              const spriteY = getScaledWeaponY();
              this._3dWeaponSprites['left'] = new Sprite_3DWeapon(leftWeapon, spriteX, spriteY);
            }
            if (this._3dWeaponSprites['left']._model) {
              this._3dWeaponSprites['left']._model.visible = true;
              this._3dWeaponSprites['left']._visible = true;
            }
          } else {
            if (this._3dWeaponSprites['left']) {
              this._3dWeaponSprites['left'].terminate();
              delete this._3dWeaponSprites['left'];
            }
          }
        } else {
          if (this._3dWeaponSprites) {
            for (const key in this._3dWeaponSprites) {
              this._3dWeaponSprites[key].terminate();
            }
            this._3dWeaponSprites = {};
          }
        }
    ...
    // Detect when the displayed actor changes so we can slide the new weapon in
    const newActorId = actor.actorId();
    const actorChanged = this._lastWeaponActorId !== newActorId;
    this._lastWeaponActorId = newActorId;
          ...
          const leftWeaponData = {
            getWeaponSpriteData: function () {
              ...
              return {
                ...
                animations: weapons[1].weaponAnimations || ["Swing"],  // i18n-ignore  animation clip name
              };
            },
            weapons: function () {
              return [weapons[1]];
    ...
    // When the actor changed, slide the new 2D weapon(s) in from the right
    if (actorChanged) {
      if (this._weaponSprite && this._weaponSprite.visible) {
        this._weaponSprite.startSlideIn();
      }
      if (this._weaponSpriteLeft && this._weaponSpriteLeft.visible) {
        this._weaponSpriteLeft.startSlideIn();
      }
    }
  };
  ...
  Spriteset_Battle.prototype.playWeaponAnimation = function (
    ...
    if (!isLeftHand) {
      if (actor.isWeaponThrow()) {
        ...
        if (enemies.length > 0) {
          ...
          if (targetSpriteEnemy && this._whipSprite) {
        ...
        if (enemies.length > 0) {
          const target = enemies[0];
          const targetSpriteEnemy = this.findTargetSprite(target);
          if (targetSpriteEnemy && this._flailSprite) {
            const targetX = targetSpriteEnemy.x;
            const targetY = targetSpriteEnemy.y - targetSpriteEnemy.height / 2;
        ...
        if (enemies.length > 0) {
          ...
          if (targetSpriteEnemy) {
            const targetX = targetSpriteEnemy.x;
            const targetY = targetSpriteEnemy.y - targetSpriteEnemy.height / 2;
            const nSprite = this.getSpecialWeaponSprite('nunchaku');
            if (nSprite) nSprite.swing(targetX, targetY);
            debugLog("Nunchaku swinging towards enemy at", targetX, targetY);
          }
        }
      } else if (actor.isWeaponBow()) {
        if (this._bowSprite) this._bowSprite.drawBow();
        ...
        // LEFT HAND NUNCHAKU
          ...
          if (enemies.length > 0) {
            ...
            if (targetSpriteEnemy) {
              ...
              const targetY =
                targetSpriteEnemy.y - targetSpriteEnemy.height / 2;
              const nSpriteLeft = this.getSpecialWeaponSprite('nunchaku', true);
              if (nSpriteLeft) nSpriteLeft.swing(targetX, targetY);
              debugLog(
    ...
    // 3D weapon sprite animation trigger
    if (ConfigManager.weaponSprites3D && window.Sprite_3DWeapon) {
      if (window.WeaponSystemProcedural) WeaponSystemProcedural.patchSprite3DWeapon();
      ...
      if (weapon) {
        ...
        if (!this._3dWeaponSprites[key] || this._3dWeaponSprites[key]._weapon !== weapon) {
          if (this._3dWeaponSprites[key]) {
             // Use terminate() (runs disposeWeaponObject3D) like every other swap
             // path; a bare scene.remove leaks the model's GPU buffers each swap.
             this._3dWeaponSprites[key].terminate();
          }
          const spriteX = getScaledWeaponX(isLeftHand);
          const spriteY = getScaledWeaponY();
          this._3dWeaponSprites[key] = new Sprite_3DWeapon(weapon, spriteX, spriteY);
        }
        const animName = (weapon.weaponAnimations && weapon.weaponAnimations[0]) || 'Swing';  // i18n-ignore  animation clip name
        this._3dWeaponSprites[key].playAnimation(animationOverride || animName);
      }
    }
  };
  ...
  Spriteset_Battle.prototype.update = function () {
    ...
    // at least one weapon model is visible.
    if (anyModelVisible && window.WeaponThreeScene &&
  ...
  Scene_Boot.prototype.start = function () {
    ...
    if (debugMode) {
      ...
      console.log(
        ...
        Object.keys(MovementKeyFrame).concat(["Static"])  // i18n-ignore  animation clip name
      );
    }
  };
  ...
  Scene_Battle.prototype.terminate = function () {
    ...
    if (this._spriteset) {
      ...
      if (this._spriteset._3dWeaponSprites) {
        for (const key in this._spriteset._3dWeaponSprites) {
          this._spriteset._3dWeaponSprites[key].terminate();
        }
        this._spriteset._3dWeaponSprites = {};
      }
    }
  };
