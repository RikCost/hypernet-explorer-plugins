...
(() => {
    ...
    const PIECE_SYMBOLS = {
        ...
        [PIECES.EMOJI]: '' // Chaos pieces are drawn as icons, not as text
    };

    // Chaos pieces the cheating AI drops on the board. Each one is a real
    // IconSet entry (indices per js/db/Sprites/Icons.json) drawn with drawIcon,
    // and its name is what shows up in the move log.
    const CHAOS_PIECES = [
        { icon: 218, key: 'bomb' },
        { icon: 267, key: 'egg' },
        { icon: 261, key: 'mushroom' },
        { icon: 210, key: 'chest' },
        { icon: 253, key: 'fish' },
        { icon: 205, key: 'campfire' },
        { icon: 219, key: 'stew' },
        { icon: 208, key: 'jug' },
        { icon: 214, key: 'candle' },
        { icon: 273, key: 'soda' }
    ...
    class ChessGame {
        constructor() {
            ...
            this.moveLog = [];
            this.chaosPieces = new Map(); // Chaos square key -> IconSet index
        ...
        getPieceSymbol(piece, row, col) {
            if (piece === PIECES.EMOJI) {
                // Text contexts (move log) go through drawTextEx, which renders
                // the \I[n] escape as the actual icon.
                const icon = this.getChaosIcon(row, col);
                return icon ? `\\I[${icon}]` : '';
            }
            return PIECE_SYMBOLS[piece];
        }

        // IconSet index of the chaos piece standing on a square, if any.
        getChaosIcon(row, col) {
            return this.chaosPieces.get(`${row}-${col}`) || 0;
        }
        ...
        isLegalMove(fromRow, fromCol, toRow, toCol) {
            ...
            if (this.currentPlayer === 'white') {
                ...
                // Emoji (chaos) pieces belong to black: they may move as black and
                // must not be captured by black as if they were the enemy.
                if (this.isBlackPiece(targetPiece) || targetPiece === PIECES.EMOJI) return false;
                if (!this.isBlackPiece(piece) && piece !== PIECES.EMOJI && piece !== PIECES.EMPTY) return false;
            }
        ...
        makeMove(fromRow, fromCol, toRow, toCol) {
            ...
            if (this.gameMode === 'chaos' && this.currentPlayer === 'white' && capturedPiece !== PIECES.EMPTY) {
                ...
                this.addToLog(T('Chess.cheatMeterGain', { gain: cheatGain, meter: this.cheatMeter, max: this.maxCheatMeter }));
            }
            
            this.board[toRow][toCol] = piece;
            this.board[fromRow][fromCol] = PIECES.EMPTY;
            
            // Handle chaos piece movement
            if (piece === PIECES.EMOJI) {
                const chaosIcon = this.chaosPieces.get(`${fromRow}-${fromCol}`);
                this.chaosPieces.delete(`${fromRow}-${fromCol}`);
                if (chaosIcon) {
                    this.chaosPieces.set(`${toRow}-${toCol}`, chaosIcon);
                }
            }
        ...
        makeNormalAIMove() {
            ...
            if (validMoves.length === 0) {
                this.addToLog(T('Chess.blackStuck'));
                this.gameEnded = true;
                if (window.MinigameFun) window.MinigameFun.won('Chess');
                return;
            }
            ...
            this.addToLog(T('Chess.blackPlays', { move: moveNotation }));
        }
        ...
        chaosNormalMove() {
            ...
            if (validMoves.length > 0) {
                ...
                this.addToLog(T('Chess.blackPlaysNormally', { move: moveNotation }));
            } else {
                this.chaosColorChange();
            }
        }
        ...
        chaosColorChange() {
            ...
            if (whitePieces.length > 0) {
                ...
                const messages = T.list('Chess.taunt.colorChange', { piece: PIECE_SYMBOLS[target.piece], square: String.fromCharCode(97 + target.col) + (8 - target.row) });
        ...
        chaosMovePlayerPiece() {
            ...
            if (whitePieces.length > 0) {
                ...
                if (emptySquares.length > 0) {
                    ...
                    const messages = T.list('Chess.taunt.relocate', { piece: PIECE_SYMBOLS[piece.piece], square: String.fromCharCode(97 + target.col) + (8 - target.row) });
                    this.addToLog(messages[Math.floor(Math.random() * messages.length)]);
                } else {
                    this.chaosTransformPiece();
                }
        ...
        chaosTransformPiece() {
            ...
            if (allPieces.length > 0) {
                ...
                const messages = T.list('Chess.taunt.transform', { from: PIECE_SYMBOLS[oldPiece], to: PIECE_SYMBOLS[newPiece] });
                this.addToLog(messages[Math.floor(Math.random() * messages.length)]);
            } else {
                this.chaosCreateEmoji();
            }
        ...
        chaosResurrectPiece() {
            ...
            if (emptySquares.length > 0) {
                ...
                const messages = T.list('Chess.taunt.spawnPiece', { piece: PIECE_SYMBOLS[newPiece], square: String.fromCharCode(97 + target.col) + (8 - target.row) });
                this.addToLog(messages[Math.floor(Math.random() * messages.length)]);
            } else {
                this.chaosNormalMove();
            }
        ...
        chaosCreateEmoji() {
            ...
            if (emptySquares.length > 0) {
                const target = emptySquares[Math.floor(Math.random() * emptySquares.length)];
                const chaos = CHAOS_PIECES[Math.floor(Math.random() * CHAOS_PIECES.length)];
                this.board[target.row][target.col] = PIECES.EMOJI;
                this.chaosPieces.set(`${target.row}-${target.col}`, chaos.icon);
                // The log window draws with drawTextEx, so \I[n] shows the icon.
                const piece = `\\I[${chaos.icon}]` + T('Chess.chaosPiece.' + chaos.key);
                
                const messages = T.list('Chess.taunt.spawnChaos', { piece: piece, square: String.fromCharCode(97 + target.col) + (8 - target.row) });
                this.addToLog(messages[Math.floor(Math.random() * messages.length)]);
            } else {
                this.chaosTransformPiece();
            }
        ...
        chaosStealCheatMeter() {
            if (this.cheatMeter > 20) {
                ...
                const messages = T.list('Chess.taunt.drain', { amount: stolen });
                this.addToLog(messages[Math.floor(Math.random() * messages.length)]);
            } else {
                this.chaosNormalMove();
            }
        ...
        chaosMultiMove() {
            ...
            const introMessages = T.list('Chess.taunt.multiMove', { moves: moves });
            ...
            for (let i = 0; i < moves; i++) {
                ...
                if (validMoves.length > 0) {
                    ...
                    this.addToLog('  ' + T('Chess.multiMoveStep', { index: i + 1, total: moves, move: moveNotation }));
                } else {
                    break;
                }
            }
        ...
        checkGameEnd() {
            ...
            if (whiteKings === 0 && whitePieces === 0) {
                this.addToLog(T('Chess.whiteWiped'));
                this.gameEnded = true;
                if (window.MinigameFun) window.MinigameFun.lost('Chess');
            } else if (blackKings === 0 && blackPieces === 0) {
                this.addToLog(T('Chess.blackWiped'));
                this.gameEnded = true;
                if (window.MinigameFun) window.MinigameFun.won('Chess');
            }
        }
    ...
    class Window_CheatMeter extends Window_Base {
        ...
        refresh() {
            // Skip the clear+redraw unless a value that drives the meter changed.
            const c = this.chess;
            const sig = (!c || c.gameMode !== 'chaos')
                ? 'none'
                : [c.cheatMeter, c.maxCheatMeter, c.canActivateCheatMode(),
                   c.isPlayerCheating(), c.cheatMoves].join('|');
            if (this._meterSig === sig) return;
            this._meterSig = sig;
            ...
            this.contents.drawText(T('Chess.cheatMeterTitle'), 0, 0, this.contents.width, 24, 'center');
            ...
            if (this.chess.canActivateCheatMode()) {
                ...
                this.contents.drawText(T('Chess.pressShift'), 0, meterY + meterHeight + 25, this.contents.width, 24, 'center');
                this.contents.textColor = this.contents.normalColor();
            } else if (this.chess.isPlayerCheating()) {
                this.contents.textColor = '#ff4444';
                this.contents.drawText(T('Chess.cheatModeLeft', { moves: this.chess.cheatMoves }), 0, meterY + meterHeight + 25, this.contents.width, 24, 'center');
                this.contents.textColor = this.contents.normalColor();
            } else {
                this.contents.drawText(T('Chess.chargeHint'), 0, meterY + meterHeight + 25, this.contents.width, 24, 'center');
            }
        }
    }
    ...
    class Scene_Chess extends Scene_MenuBase {
        ...
        createModeWindow() {
            ...
            const text = T('Chess.mode', { mode: this.chess.gameMode === 'normal' ? T('Chess.modeNormal') : T('Chess.modeChaos') });
    ...
    class Window_ChessBoard extends Window_Selectable {
        ...
        destroy(options) {
            // Cancel any pending AI move so it cannot fire against a dead scene.
            if (this._aiMoveTimer) {
        ...
        drawItem(index) {
            ...
            if (this.chess) {
                ...
                if (piece === PIECES.EMOJI) {
                    // Chaos pieces are IconSet items: blit the icon instead of
                    // drawing a glyph, centred in the square.
                    const icon = this.chess.getChaosIcon(row, col);
                    if (icon) {
                        const iw = ImageManager.iconWidth;
                        const ih = ImageManager.iconHeight;
                        this.drawIcon(icon, rect.x + (rect.width - iw) / 2, rect.y + (rect.height - ih) / 2);
                    }
                ...
                if (this.chess.currentPlayer === 'white') {
                    ...
                    if (moveCheck.legal || moveCheck.cheat) {
                        ...
                        if (moveCheck.cheat) {
                            ...
                            this.chess.addToLog(T('Chess.whiteCheats', { move: moveNotation }));
                        } else {
                            this.chess.addToLog(T('Chess.whitePlays', { move: moveNotation }));
                        }
                        ...
                        if (!this.chess.gameEnded) {
                            // AI move
                            this._aiMoveTimer = setTimeout(() => {
                                this._aiMoveTimer = null;
                                // Bail if the scene was popped during the delay,
                                // otherwise refresh()/logWindow deref a dead scene.
                                if (!(SceneManager._scene instanceof Scene_Chess)) return;
    ...
    Window_MenuCommand.prototype.addOriginalCommands = function() {
        ...
        this.addCommand(T('Chess.menuCommand'), 'chess');
    };
})();