...
(() => {
  ...
  class Window_WorkJobDetails extends Window_Base {
    ...
    refresh() {
      ...
      this.resetTextColor();
      this.drawText(T('WorkSystem.category', { category: job.category }), 0, y, 300);
      this.drawText(T('WorkSystem.durationHours', { hours: job.duration }), 310, y, 200);
      y += lineHeight + 4;

      // Requirements header
      this.changeTextColor(ColorManager.systemColor());
      this.drawText(T('WorkSystem.requirementsHeader'), 0, y, this.contents.width);
      y += lineHeight;
      ...
      for (const [stat, required] of Object.entries(job.requirements)) {
        ...
        this.drawText(`${_si18n(statKeyMapping[stat] || stat)}: ${actorValue} / ${required}`, 20, y, 200);
        y += lineHeight;
      }
      ...
      if (settings.showSuccessChance) {
        ...
        this.changeTextColor(ColorManager.systemColor());
        this.drawText(T('WorkSystem.successRateHeader'), 0, y, 150);
      ...
      if (!reqCheck.meets) {
        this.changeTextColor(ColorManager.deathColor());
        this.drawText(T('WorkSystem.warnStats'), 0, y, this.contents.width, 'center');
        y += lineHeight;
        this.resetTextColor();
        this.drawText(T('WorkSystem.warnRisk'), 0, y, this.contents.width, 'center');
        y += lineHeight;
      }
      ...
      if (job.factionId !== undefined) {
        y += 4;
        this.changeTextColor(ColorManager.systemColor());
        this.drawText(T('WorkSystem.factionJobId', { id: job.factionId }), 0, y, this.contents.width);
      }
    }
  }
